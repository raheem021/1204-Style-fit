  ...
  return (
    <form onSubmit={handleSubmit}>
      <div style={{ marginBottom: '1.5rem' }}>
        <label style={{ display: 'block', fontWeight: '700', fontSize: '0.8rem', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '0.75rem' }}>
          Card Details
        </label>
        <div style={{ padding: '1rem', border: '1px solid #e0e0e0', borderRadius: '4px', background: '#fafafa' }}>
          <CardElement options={{
            style: {
      ...
      <button type="submit" disabled={!stripe || loading} style={{
        width: '100%', padding: '1.25rem', background: loading ? '#666' : '#111',
        color: '#fff', border: 'none', fontSize: '0.9rem',
        fontWeight: '800', letterSpacing: '3px', cursor: loading ? 'not-allowed' : 'pointer',
        borderRadius: '2px', textTransform: 'uppercase',
      }}>
        {loading ? 'Processing...' : `Pay $${amount.toFixed(2)}`}
      </button>

      <p style={{ textAlign: 'center', color: '#999', fontSize: '0.75rem', marginTop: '0.75rem' }}>
