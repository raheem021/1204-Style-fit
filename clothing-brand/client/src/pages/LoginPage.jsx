  ...
  return (
    <div style={{ maxWidth: '400px', margin: '4rem auto', padding: '2rem', background: '#fff', borderRadius: '8px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
      ...
      <form onSubmit={handleSubmit}>
        {['email', 'password'].map(field => (
          <div key={field} style={{ marginBottom: '1rem' }}>
            ...
            <input name={field} type={field} value={form[field]} onChange={handleChange} required
      ...
      <p style={{ marginTop: '1rem', textAlign: 'center' }}>
        Don't have an account? <Link to="/register" style={{ color: '#111', fontWeight: 'bold' }}>Register</Link>
      </p>
    </div>
  );
}