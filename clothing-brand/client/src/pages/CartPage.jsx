          ...
          <div>
            {cartItems.map((item, i) => (
              <div key={`${item._id}-${item.size}`} style={{
                ...
                marginBottom: '1rem', boxShadow: '0 2px 16px rgba(0,0,0,0.06)',
              }}>
                <img src={item.images?.[0]} alt={item.name}
                  style={{ width: '100px', height: '120px', objectFit: 'cover', borderRadius: '4px', flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: '0.7rem', color: '#999', textTransform: 'uppercase', letterSpacing: '2px', margin: '0 0 0.3rem' }}>{item.category}</p>
                  <h3 style={{ margin: '0 0 0.5rem', fontWeight: '800', fontSize: '1.1rem' }}>{item.name}</h3>
                  <p style={{ margin: '0 0 0.75rem', color: '#666', fontSize: '0.85rem' }}>Size: <strong>{item.size}</strong></p>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      ...
                      <button onClick={() => removeFromCart(item._id, item.size)} style={{
            ...
            <Link to="/products" style={{ color: '#111', fontSize: '0.85rem', fontWeight: '700', letterSpacing: '2px', textDecoration: 'none', borderBottom: '2px solid #111', paddingBottom: '2px' }}>
              ← CONTINUE SHOPPING
            </Link>
          </div>

          {/* Order Summary */}
          <div style={{ background: '#fff', borderRadius: '4px', padding: '2rem', boxShadow: '0 2px 16px rgba(0,0,0,0.06)', position: 'sticky', top: '2rem' }}>
            <h2 style={{ fontWeight: '900', fontSize: '1.3rem', marginBottom: '1.5rem', letterSpacing: '-0.5px' }}>Order Summary</h2>

            {cartItems.map(item => (
              <div key={`${item._id}-${item.size}`} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem', fontSize: '0.9rem' }}>
            ...
            <div style={{ borderTop: '1px solid #eee', margin: '1.5rem 0', paddingTop: '1.5rem' }}>
              ...
              {totalPrice < 100 && (
                <p style={{ color: '#f0c040', fontSize: '0.78rem', fontWeight: '700', background: '#fffbea', padding: '0.6rem', borderRadius: '4px', margin: '0.5rem 0' }}>
            ...
            <button onClick={() => navigate('/checkout')} style={{
              width: '100%', padding: '1.25rem', background: '#111',
